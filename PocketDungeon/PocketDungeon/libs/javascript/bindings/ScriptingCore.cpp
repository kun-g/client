...
JSBool ScriptingCore::executeFunctionWithThis(jsval thiz, const char *name, uint32_t argc /* = 0 */, jsval *vp /* = NULL */, jsval* retVal /* = NULL */)
{
    JSBool bRet = JS_FALSE;
    JSContext* cx = this->cx_;
    JSObject* obj = JSVAL_TO_OBJECT(thiz);
...
int ScriptingCore::executeEvent(int nHandler, const char* pEventName, CCObject* pEventSource/* = NULL*/, const char* pEventSourceClassName/* = NULL*/)
{
    ...
    {
        js_proxy_t * p = jsb_get_native_proxy(pEventSource);
        if (!p) return 0;
        
        jsval retval;
        jsval dataVal = JSVAL_NULL;
        
        executeFunctionWithOwner(OBJECT_TO_JSVAL(p->obj), pEventName, 1, &dataVal, &retval);
        return 1;
    }
    return 0;
}
