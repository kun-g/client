...
function pushForgeAnimation(item, func, obj){
    var layer = engine.ui.newLayer();
    var mask = blackMask();
    layer.addChild(mask);
    var winSize = cc.Director.getInstance().getWinSize();
    theForgeAnimationNode = libEffect.attachEffectCCBI(layer, cc.p(winSize.width/2, winSize.height/2), "effect-forge.ccbi", libEffect.EFFECTMODE_STAY);
    var icon = libItem.UIItem.create(item);
    theForgeAnimationNode.owner.nodeItem.addChild(icon);
...
function onStartUpgrade(sender){
    ...
    if( UpgradeArgs != null ){
        libUIKit.waitRPC(Request_InventoryUseItem, UpgradeArgs, function(rsp){
            if( rsp.RET == RET_OK ){
                pushForgeAnimation(theForgeItem, function(){
...
function onForgeIngredient(sender){
    ...
    theSSList = list.sort(function(a, b){
        var va = -a.ClassId;
        var ia = engine.user.inventory.Items[a.Items[0]];
        if( !ia.isAvailable() ){
            va += 100000;
        }
        var vb = -b.ClassId;
        var ib = engine.user.inventory.Items[b.Items[0]];
        if( !ib.isAvailable() ){
            vb += 100000;
        }
        return va - vb;
    ...
    var recipeSize = cc.size(0, content.ui.recipeList.getViewSize().height);
    if( theSSList.length > 0 ){
        for(var k in theSSList){
            var NK = Number(k);
            var info = theSSList[k];
        ...
        label.setAnchorPoint(cc.p(0.5 ,0.5));
        var viewSize = content.ui.recipeList.getViewSize();
        label.setPosition(cc.p(viewSize.width/2, viewSize.height/2));
        theRecipeLayer.addChild(label);
    }
    theRecipeLayer.setContentSize(recipeSize);
    content.ui.recipeList.setContainer(theRecipeLayer);
...
function setForgeRecipe(info){
    ...
    if( targetClass.rank != null ){
        theContent.owner.labRank.setString(targetClass.rank);
    }
...
function loadExtractItems(content){
    ...
    content.node.addChild(ExtractSlider);
}
...
function onNotify(ntf){
    switch(ntf.NTF){
        ...
        {
            theContent.ui.treasureDisplay.setTreasure(engine.user.inventory.Gold, engine.user.inventory.Diamond);
            theContent.ui.treasureDisplayL.setTreasure(engine.user.inventory.Gold, engine.user.inventory.Diamond);
            theContent.ui.treasureDisplayR.setTreasure(engine.user.inventory.Gold, engine.user.inventory.Diamond);
            return false;
        }
