...
function doDungeonResult(win){
    ...
    {//lose
        ...
        var actSeq = cc.Sequence.create(actDelay, actFunc);
        theLayer.runAction(actSeq);
        theLayer.mask.runAction(cc.FadeIn.create(3));
    }

    if( theDungeon.TutorialFlag ){
        engine.event.sendNTFEvent(Request_TutorialStageComplete, {
            stg: 2
        });
...
function onEnter()
{
    ...
    //theLayer.mask.runAction(cc.FadeOut.create(2));//test
...
function addActor(unit, boss)
{
    ...
    {
        actor = new avatar.Avatar(unit.uuid);
    }
