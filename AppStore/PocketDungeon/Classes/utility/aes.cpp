...
uint gKS[60*4];
int gKL = 0;
const char * KEYDIC = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
...
void AESKEY(const char* k)
{
    ...
    uchar * keys = (uchar*)malloc(gKL/8);
    memset(keys, (int)' ', gKL/8);
    size_t keylength = strlen(k);
    if( keylength > gKL/8 )
    {
        keylength = gKL/8;
    }
    memcpy(keys, k, keylength);
    KeyExpansion(keys, gKS, gKL);
    free(keys);
}
...
{
    size_t round = resized/16;
    for(int i=0; i<round; ++i)
    {
        aes_decrypt(&in[16*i], &out[16*i], gKS, gKL);
    }
}
...
void ENAES(unsigned char in[], unsigned char out[], unsigned int resized)
{
    ...
    for(int i=0; i<round; ++i)
    {
        aes_encrypt(&in[16*i], &out[16*i], gKS, gKL);
    }
}
