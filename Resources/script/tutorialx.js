...
var TutorialPress = cc.Layer.extend({
    ...
    setClick: function(pos){
        libEffect.attachEffectCCBI(this, pos, "tips-hand.ccbi", libEffect.EFFECTMODE_LOOP);
    },
    setSlide: function(pos){
        var node = libEffect.attachEffectCCBI(this, pos, "tips-hand.ccbi", libEffect.EFFECTMODE_LOOP);
        node.stopAllActions();
        var mv1 = cc.MoveBy.create(0.8, cc.p(0, 100));
        var mv2 = cc.MoveBy.create(0.1, cc.p(0, -100));
...
TutorialManager.prototype.nextAction = function(ui){
    if( this.TUTORIAL != null ){
        ...
        if( this.TUTORIAL.actions.length > this.ACTION ){
            ...
            switch(act.act){
                ...
                case "pressclick":{
                    var pressclick = TutorialPress.create(act.rect, act.trigger);
                    pressclick.setClick(act.pos);
                    engine.ui.curLayer.addChild(pressclick);
                }break;
                case "pressslide":{
                    var pressclick = TutorialPress.create(act.rect, act.trigger);
                    pressclick.setSlide(act.pos);
                    engine.ui.curLayer.addChild(pressclick);
                }break;
                case "yield":{
                    return false;
                }break;
                default: return this.nextAction(ui);
            }
            return true;
        }
    }
