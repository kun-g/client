...
//global cached data
std::map<std::string, CCData*> gCCBCaches;//hammer tweaks: cache
...
void CCBReader::purageCache()
{
    CCLog("PURAGE CCB CACHE");
    std::map<std::string, CCData*>::iterator iter;
    for(iter = gCCBCaches.begin(); iter != gCCBCaches.end(); ++iter)
    {
        CCData* data = iter->second;
        data->release();
    }
    gCCBCaches.clear();
}
...
CCNode* CCBReader::readNodeGraphFromFile(const char *pCCBFileName, CCObject *pOwner, const CCSize &parentSize)
{
    ...
    //hammer tweaks: cache
    CCData *data = NULL;
    std::map<std::string, CCData*>::iterator found = gCCBCaches.find(strCCBFileName);
    if( found == gCCBCaches.end() )
    {//not found
        ...
        //put data into cache
        gCCBCaches.insert(std::pair<std::string, CCData*>(strCCBFileName, data));
    }
    else
    {//found
        data = found->second;
...
CCBKeyframe* CCBReader::readKeyframe(int type)
{
    ...
    else if (type == kCCBPropTypeSpriteFrame)
    {
        ...
        else
        {
            ...
            {
                std::string spriteSheetFile = spriteSheet;
                cutPathByLastSlash(spriteSheetFile);
                frameCache->addSpriteFramesWithFile(spriteSheetFile.c_str());
