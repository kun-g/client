...
//hamemr cut file name by last "/"
void cutPathByLastSlash(string& path)
{
    //CCLOG("PATH = %s", path.c_str());//test
    size_t cutBegin = path.rfind("/");
    if( cutBegin != std::string::npos ){
        path = path.substr(cutBegin+1, std::string::npos);
    }
}
...
CCSpriteFrame * CCNodeLoader::parsePropTypeSpriteFrame(CCNode * pNode, CCNode * pParent, CCBReader * pCCBReader, const char *pPropertyName) 
{
    ...
    if( spriteFrame == NULL )
    {
        if( spriteFile.length() != 0 || spriteSheet.length() != 0 ){
            CCLOG("NULL SpriteFrame: %s : %s", spriteSheet.c_str(), spriteFile.c_str());
        }
    }
    return spriteFrame;
}
...
CCNode * CCNodeLoader::parsePropTypeCCBFile(CCNode * pNode, CCNode * pParent, CCBReader * pCCBReader) {
    ...
    cutPathByLastSlash(ccbFileName);
