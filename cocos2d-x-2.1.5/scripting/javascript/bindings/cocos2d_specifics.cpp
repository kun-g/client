	...
	if (argc == 2) {
		...
		if (jsproxy) {
			...
			if (nproxy) {
                ...
                JS_AddNamedObjectRoot(cx, &jsproxy->obj, typeid(*((CCObject*)jsproxy->ptr)).name());
			}
		}
	}
	return JS_TRUE;
...
//填坑
void JSCCBAnimCallbackWrapper::setJSCallbackThisFromNative(CCObject *pObj)
{
    if( pObj != NULL )
    {
        js_proxy_t *proxy;
        HASH_FIND_PTR(_native_js_global_ht, &pObj, proxy);
    }
    else{
        jsThisObj = JSVAL_NULL;
    }
}

void JSCCBAnimCallbackWrapper::setJSCallbackFunctionName(const char *pFuncName)
{
    mFunctionName = std::string(pFuncName);
}

void JSCCBAnimCallbackWrapper::callbackFunc() const
{
    JSObject* thisObj = JSVAL_IS_VOID(jsThisObj) ? NULL : JSVAL_TO_OBJECT(jsThisObj);
    JSContext *cx = ScriptingCore::getInstance()->getGlobalContext();
    
    jsval retval;
    JS_CallFunctionName(cx, thisObj, mFunctionName.c_str(), 0, NULL, &retval);
}
