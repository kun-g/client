...
JSBool js_cocos2dx_extension_CCEditBox_makeTextAlignCenter(JSContext *cx, uint32_t argc, jsval *vp)
{
	JSObject *obj = JS_THIS_OBJECT(cx, vp);
	js_proxy_t *proxy = jsb_get_js_proxy(obj);
	cocos2d::extension::CCEditBox* cobj = (cocos2d::extension::CCEditBox *)(proxy ? proxy->ptr : NULL);
...
void js_register_cocos2dx_extension_CCEditBox(JSContext *cx, JSObject *global) {
	...
	static JSFunctionSpec funcs[] = {
        ...
        JS_FN("makeTextAlignCenter", js_cocos2dx_extension_CCEditBox_makeTextAlignCenter, 0, JSPROP_PERMANENT | JSPROP_ENUMERATE),
...
JSClass  *jsb_CCRectClip_class;
JSObject *jsb_CCRectClip_prototype;

JSBool js_cocos2dx_extension_CCRectClip_init(JSContext *cx, uint32_t argc, jsval *vp)
{
	JSObject *obj = JS_THIS_OBJECT(cx, vp);
	js_proxy_t *proxy = jsb_get_js_proxy(obj);
	cocos2d::extension::CCRectClip* cobj = (cocos2d::extension::CCRectClip *)(proxy ? proxy->ptr : NULL);
	JSB_PRECONDITION2( cobj, cx, JS_FALSE, "Invalid Native Object");
	if (argc == 0) {
		bool ret = cobj->init();
		jsval jsret;
...
JSBool js_cocos2dx_extension_CCRectClip_getClipRect(JSContext *cx, uint32_t argc, jsval *vp)
{
	JSObject *obj = JS_THIS_OBJECT(cx, vp);
	js_proxy_t *proxy = jsb_get_js_proxy(obj);
	cocos2d::extension::CCRectClip* cobj = (cocos2d::extension::CCRectClip *)(proxy ? proxy->ptr : NULL);
	JSB_PRECONDITION2( cobj, cx, JS_FALSE, "Invalid Native Object");
	if (argc == 0) {
		cocos2d::CCRect ret = cobj->getClipRect();
		jsval jsret = ccrect_to_jsval(cx, ret);
		JS_SET_RVAL(cx, vp, jsret);		return JS_TRUE;
	}

	JS_ReportError(cx, "wrong number of arguments: %d, was expecting %d", argc, 0);
	return JS_FALSE;
}
JSBool js_cocos2dx_extension_CCRectClip_setClipRect(JSContext *cx, uint32_t argc, jsval *vp)
{
	jsval *argv = JS_ARGV(cx, vp);
	JSBool ok = JS_TRUE;
	JSObject *obj = JS_THIS_OBJECT(cx, vp);
	...
	if (argc == 1) {
		cocos2d::CCRect arg0;
        jsval_to_ccrect(cx, argv[0], &arg0);
		JSB_PRECONDITION2(ok, cx, JS_FALSE, "Error processing arguments");
		cobj->setClipRect(arg0);
		JS_SET_RVAL(cx, vp, JSVAL_VOID);
		return JS_TRUE;
	}
...
JSBool js_cocos2dx_extension_CCRectClip_create(JSContext *cx, uint32_t argc, jsval *vp)
{
	if (argc == 0) {
		cocos2d::extension::CCRectClip* ret = cocos2d::extension::CCRectClip::create();
		jsval jsret;
		do {
		if (ret) {
			js_proxy_t *proxy = js_get_or_create_proxy<cocos2d::extension::CCRectClip>(cx, ret);
			jsret = OBJECT_TO_JSVAL(proxy->obj);
		} else {
			jsret = JSVAL_NULL;
		}
...
void js_cocos2dx_extension_CCRectClip_finalize(JSFreeOp *fop, JSObject *obj) {
    CCLOGINFO("jsbindings: finalizing JS object %p (CCRectClip)", obj);
}

static JSBool js_cocos2dx_extension_CCRectClip_ctor(JSContext *cx, uint32_t argc, jsval *vp)
{
	JSObject *obj = JS_THIS_OBJECT(cx, vp);
    cocos2d::extension::CCRectClip *nobj = new cocos2d::extension::CCRectClip();
    js_proxy_t* p = jsb_new_proxy(nobj, obj);
    nobj->autorelease();
    JS_AddNamedObjectRoot(cx, &p->obj, "cocos2d::extension::CCRectClip");
    JS_SET_RVAL(cx, vp, JSVAL_VOID);
    return JS_TRUE;
}

void js_register_cocos2dx_extension_CCRectClip(JSContext *cx, JSObject *global) {
	jsb_CCRectClip_class = (JSClass *)calloc(1, sizeof(JSClass));
	jsb_CCRectClip_class->name = "RectClip";
	jsb_CCRectClip_class->addProperty = JS_PropertyStub;
	jsb_CCRectClip_class->delProperty = JS_PropertyStub;
	jsb_CCRectClip_class->getProperty = JS_PropertyStub;
	jsb_CCRectClip_class->setProperty = JS_StrictPropertyStub;
	jsb_CCRectClip_class->enumerate = JS_EnumerateStub;
	jsb_CCRectClip_class->resolve = JS_ResolveStub;
	jsb_CCRectClip_class->convert = JS_ConvertStub;
	jsb_CCRectClip_class->finalize = js_cocos2dx_extension_CCRectClip_finalize;
	jsb_CCRectClip_class->flags = JSCLASS_HAS_RESERVED_SLOTS(2);
	...
	static JSFunctionSpec funcs[] = {
		JS_FN("init", js_cocos2dx_extension_CCRectClip_init, 0, JSPROP_PERMANENT | JSPROP_ENUMERATE),
		JS_FN("getClipRect", js_cocos2dx_extension_CCRectClip_getClipRect, 0, JSPROP_PERMANENT | JSPROP_ENUMERATE),
		JS_FN("setClipRect", js_cocos2dx_extension_CCRectClip_setClipRect, 1, JSPROP_PERMANENT | JSPROP_ENUMERATE),
        JS_FN("ctor", js_cocos2dx_extension_CCRectClip_ctor, 0, JSPROP_PERMANENT | JSPROP_ENUMERATE),
        JS_FS_END
	};

	static JSFunctionSpec st_funcs[] = {
		JS_FN("create", js_cocos2dx_extension_CCRectClip_create, 0, JSPROP_PERMANENT | JSPROP_ENUMERATE),
		JS_FS_END
	};

	jsb_CCRectClip_prototype = JS_InitClass(
		cx, global,
		jsb_CCNode_prototype,
		jsb_CCRectClip_class,
		dummy_constructor<cocos2d::extension::CCRectClip>, 0, // no constructor
		properties,
		funcs,
		NULL, // no static properties
		st_funcs);
	// make the class enumerable in the registered namespace
	JSBool found;
	JS_SetPropertyAttributes(cx, global, "RectClip", JSPROP_ENUMERATE | JSPROP_READONLY, &found);

	// add the proto and JSClass to the type->js info hash table
	TypeTest<cocos2d::extension::CCRectClip> t;
	js_type_class_t *p;
	uint32_t typeId = t.s_id();
	HASH_FIND_INT(_js_global_type_ht, &typeId, p);
	if (!p) {
		p = (js_type_class_t *)malloc(sizeof(js_type_class_t));
		p->type = typeId;
		p->jsclass = jsb_CCRectClip_class;
		p->proto = jsb_CCRectClip_prototype;
		p->parentProto = jsb_CCNode_prototype;
		HASH_ADD_INT(_js_global_type_ht, type, p);
	}
}
...
void register_all_cocos2dx_extension(JSContext* cx, JSObject* obj) {
	...
	js_register_cocos2dx_extension_CCRectClip(cx, obj);
